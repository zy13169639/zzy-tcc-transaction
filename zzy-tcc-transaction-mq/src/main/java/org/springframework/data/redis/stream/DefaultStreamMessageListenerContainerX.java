...
public class DefaultStreamMessageListenerContainerX<K, V extends Record<K, ?>> extends DefaultStreamMessageListenerContainer<K, V> {
    ...
    public Subscription register(StreamReadRequest<K> streamRequest, StreamListener<K, V> listener) {
        return this.doRegisterX(getReadTaskX1(streamRequest, listener));
    }
    ...
    private StreamPollTask<K, V> getReadTaskX1(StreamReadRequest<K> streamRequest, StreamListener<K, V> listener) {
        StreamPollTask<K, V> task = ReflectUtil.invoke(this, "getReadTask", streamRequest, listener);
        // 修改 readFunction 方法
        BiFunction<K, ReadOffset, List<V>> readFunction = (BiFunction<K, ReadOffset, List<V>> ) ReflectUtil.getFieldValue(task, "readFunction");
        ReflectUtil.setFieldValue(task, "readFunction",  (BiFunction<K, ReadOffset, List<V>>) (K key,ReadOffset readOffset)-> {
            List<V> records = readFunction.apply(key,readOffset);
            //【重点】保证 records 不是空，避免 NPE 的问题！！！
            return records != null ? records : Collections.emptyList();
        });
        return task;
